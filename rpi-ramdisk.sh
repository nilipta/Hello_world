#!/bin/bash

RAMDISK_PATH="/mnt/ramdisk"

# Get total RAM in KB
TOTAL_RAM_KB=$(grep MemTotal /proc/meminfo | awk '{print $2}')

# Get current user UID and GID
USER_UID=$(id -u)
USER_GID=$(id -g)

echo "Choose an action:"
echo "  [m] Mount RAM disk"
echo "  [u] Unmount RAM disk"
echo "  [q] Quit"
read -p "Enter your choice [m/u/q]: " choice

case "$choice" in
    [mM])
        # Ask user for percentage
        read -p "Enter RAM allocation percentage (10–90): " ALLOC_PERCENT

        # Validate input
        if ! [[ "$ALLOC_PERCENT" =~ ^[0-9]+$ ]]; then
            echo "Error: Please enter a valid number."
            exit 1
        fi
        if [ "$ALLOC_PERCENT" -lt 10 ] || [ "$ALLOC_PERCENT" -gt 90 ]; then
            echo "Error: Percentage must be between 10 and 90."
            exit 1
        fi

        RAMDISK_SIZE_KB=$((TOTAL_RAM_KB * ALLOC_PERCENT / 100))
        RAMDISK_SIZE_MB=$((RAMDISK_SIZE_KB / 1024))

        if mountpoint -q "$RAMDISK_PATH"; then
            echo "RAM disk already mounted at $RAMDISK_PATH"
        else
            sudo mkdir -p "$RAMDISK_PATH"
            sudo mount -t tmpfs -o size=${RAMDISK_SIZE_KB}k,uid=$USER_UID,gid=$USER_GID tmpfs "$RAMDISK_PATH"
            echo "Mounted RAM disk at $RAMDISK_PATH with size ${RAMDISK_SIZE_MB}MB (${ALLOC_PERCENT}% of RAM)"
        fi
        ;;
    [uU])
        if mountpoint -q "$RAMDISK_PATH"; then
            sudo umount "$RAMDISK_PATH"
            echo "Unmounted RAM disk from $RAMDISK_PATH"
        else
            echo "No RAM disk mounted at $RAMDISK_PATH"
        fi
        ;;
    [qQ])
        echo "Exiting without changes."
        ;;
    *)
        echo "Invalid choice. Please enter m, u, or q."
        ;;
esac
